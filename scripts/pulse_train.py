...
from hunter.pulse_snapshot import COMPANIES, CUTS, SNAPSHOT_SYMBOLS     # noqa: E402
...
def gaps_de_abertura() -> pd.DataFrame:
...
# ───────────────────────── features: o snapshot ─────────────────────────
def features_do_corte(cut: str) -> pd.DataFrame:
    """Matriz data x instrumento com a variação de 24h, lida de pulse_snapshot."""
    linhas, offset = [], 0
    while True:
    ...
    px = px.reindex(columns=[s for s in SNAPSHOT_SYMBOLS if s in px.columns])
...
def main() -> int:
    ...
    ap.add_argument("--cut", choices=list(CUTS), help="treinar só um corte")
    args = ap.parse_args()
    ...
    modelos = []
    for cut in ([args.cut] if args.cut else list(CUTS)):
        X = features_do_corte(cut)
        if X.empty:
            log.error("corte %s: pulse_snapshot vazio — rode scripts/pulse_backfill.py antes", cut)
            continue
        log.info("corte %s: %d pregões x %d instrumentos", cut, X.shape[0], X.shape[1])
